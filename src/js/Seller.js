...
$(document).ready(function(){
    ...
    web3.eth.getAccounts().then(function(accounts){
      ...
      landcontract.methods.viewAssets().call({
        ...
        landcontract.methods.computeId('Maharashtra','Sangli','Miraj',12).call().then(function(id){
            landcontract.methods.makeAvailable(id).send({
                from :accounts[0]
            }).then(function(){
                console.log('success');
            })
    ...
    function viewRequestor(assest){
        landcontract.methods.viewRequest(assest).call().then(function(add){
            $("#Requestor").append(`${add}`);
            console.log(add);
        })
    }
